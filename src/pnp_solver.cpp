...
class PnPSolver{
	...
	std::vector<Eigen::Matrix<double, 4, 4>> get_all_solutions(){
		std::vector<Eigen::Matrix<double, 4, 4>> solutions;
		for (int i=0; i<solution_r.size(); i++){
			Eigen::Matrix<double, 9, 1> r = solution_r[i];
			Eigen::Matrix<double, 3, 1> t = P * r;
			solutions.push_back((Eigen::Matrix<double, 4, 4>() <<
													 r(0), r(1), r(2), t(0),
													 r(3), r(4), r(5), t(1),
													 r(6), r(7), r(8), t(2),
													 0,    0,    0,    1).finished());
